...
OUTPUT_ROOT=${OUTPUT_ROOT:-"$WORKSPACE/benchmark_results/stage_c_c5_start_development_k24_i30"}
...
WINDOW=${WINDOW:-3}
DWELL=${DWELL:-3}
MAXIMUM_DEPTH=${MAXIMUM_DEPTH:-2}
VALUES=${VALUES:-"0 3 5 7"}
OVERWRITE=${OVERWRITE:-0}
REQUEST_PORT=${REQUEST_PORT:-33220}
...
for value in $VALUES; do
  ...
  C5_WINDOW="$WINDOW" C5_DWELL="$DWELL" \
  C5_MAXIMUM_DEPTH="$MAXIMUM_DEPTH" C5_START_ITERATION="$value" \
...
echo "Stage-C C5 delayed-start sweep finished: $OUTPUT_ROOT"