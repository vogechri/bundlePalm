...
OUTPUT_ROOT=${OUTPUT_ROOT:-"$WORKSPACE/benchmark_results/k24_schur_alignment_i200"}
DATASET_LIST_FILE="$OUTPUT_ROOT/datasets.txt"
REQUEST_PORT=${REQUEST_PORT:-30920}
OVERWRITE=${OVERWRITE:-0}
PROBLEM_FILTER=${PROBLEM_FILTER:-"montreal_notre_dame piazza_del_popolo roman_forum trafalgar yorkminster 52 3068"}
SCHUR_ALIGNMENT_MAXIMUM_ITERATIONS=${SCHUR_ALIGNMENT_MAXIMUM_ITERATIONS:-500}

mkdir -p "$OUTPUT_ROOT"
one_d_sfm_manifest="$DETACHED_ROOT/serverTest/1dsfm_all_fifteen_datasets.txt"
{
  for scene in montreal_notre_dame piazza_del_popolo roman_forum trafalgar yorkminster; do
...
env \
  OUTPUT_DIR="$OUTPUT_ROOT/diagnostic" DATASET_LIST_FILE="$DATASET_LIST_FILE" \
  PROBLEM_FILTER="$PROBLEM_FILTER" \
  CLUSTERS_LIST=24 ITERATIONS=200 LOCAL_STEPS=1 THREADS_PER_CLUSTER=1 \
  ...
  SCHUR_ALIGNMENT_DIAGNOSTIC_ITERATIONS=90,120,160,200 \
  SCHUR_ALIGNMENT_CAMERA_DAMPING=0.005859375 \
  SCHUR_ALIGNMENT_LANDMARK_DAMPING=0.005859375 \
  SCHUR_ALIGNMENT_MAXIMUM_ITERATIONS="$SCHUR_ALIGNMENT_MAXIMUM_ITERATIONS" \
  ...
  BUNDLE_PALM_REQUEST_PORT="$REQUEST_PORT" BUNDLE_PALM_RESULT_PORT="$((REQUEST_PORT + 1))" \
