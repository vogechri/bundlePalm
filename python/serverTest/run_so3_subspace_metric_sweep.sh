...
ANALYZER="$SCRIPT_DIR/analyze_so3_subspace_metric_sweep.py"
PYTHON=${PYTHON:-"$SCRIPT_DIR/.venv/bin/python"}
OUTPUT_ROOT=${OUTPUT_ROOT:-"$WORKSPACE/benchmark_results/so3_subspace_metric_development_k24_i30"}
...
REQUEST_PORT=${REQUEST_PORT:-63820}
...
for value in $VALUES; do
  ...
  CAMERA_DIAGONAL_METRIC_SCALE=25 SO3_TRANSLATION_METRIC_RATIO="$value" \
  C5_START_ITERATION=5 OUTPUT_ROOT="$OUTPUT_ROOT/$tag" \
  REQUEST_PORT="$((REQUEST_PORT + 4 * index))" OVERWRITE="$OVERWRITE" \
  "$RUNNER"
  ...
  CAMERA_DIAGONAL_METRIC_SCALE=25 SO3_TRANSLATION_METRIC_RATIO="$value" \
  C5_START_ITERATION=5 OUTPUT_ROOT="$OUTPUT_ROOT/$tag" \
  REQUEST_PORT="$((REQUEST_PORT + 4 * index + 2))" OVERWRITE="$OVERWRITE" \
  "$RUNNER"
  index=$((index + 1))
done
...
echo "SO3 subspace-metric sweep finished: $OUTPUT_ROOT"
