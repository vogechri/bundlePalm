...
CLIENT=${CLIENT:-"$DETACHED_ROOT/serverTest/client_drs.py"}
...
OUTPUT_ROOT=${OUTPUT_ROOT:-"$WORKSPACE/benchmark_results/k24_quarter_i60_i90_krylov_repeat_guard"}
CONTROL_ROOT=${CONTROL_ROOT:-"$WORKSPACE/benchmark_results/k24_i200_terminal_correction_breadth"}
I60_REFERENCE_ROOT=${I60_REFERENCE_ROOT:-"$WORKSPACE/benchmark_results/k24_quarter_i60_krylov_applied_sentinel"}
UNGUARDED_REPEAT_ROOT=${UNGUARDED_REPEAT_ROOT:-"$WORKSPACE/benchmark_results/k24_quarter_i60_i90_krylov_interaction_v3"}
...
PROPOSAL_ITERATIONS=${PROPOSAL_ITERATIONS-60,90}
PROPOSAL_CAMERA_DAMPING=${PROPOSAL_CAMERA_DAMPING:-0.00146484375}
PROPOSAL_LANDMARK_DAMPING=${PROPOSAL_LANDMARK_DAMPING:-0.00146484375}
PROPOSAL_LANDMARK_REFINEMENT_STEPS=${PROPOSAL_LANDMARK_REFINEMENT_STEPS:-3}
...
if [[ -n "$PROPOSAL_ITERATIONS" ]]; then
  proposal_enabled_default=1
  proposal_direction_default=krylov2
else
  proposal_enabled_default=0
  proposal_direction_default=jacobi
fi
SCHUR_RESIDUAL_PROPOSAL_DIRECTION=${SCHUR_RESIDUAL_PROPOSAL_DIRECTION-$proposal_direction_default}
ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_REBASE_TRUST_STATE=${ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_REBASE_TRUST_STATE-$proposal_enabled_default}
SCHUR_PROPOSAL_LANDMARK_RESPONSE_ORACLE=${SCHUR_PROPOSAL_LANDMARK_RESPONSE_ORACLE-$proposal_enabled_default}
APPLY_SCHUR_PROPOSAL_LANDMARK_RESPONSE=${APPLY_SCHUR_PROPOSAL_LANDMARK_RESPONSE-$proposal_enabled_default}
ALLOW_TWO_SCHUR_RESIDUAL_PROPOSALS=${ALLOW_TWO_SCHUR_RESIDUAL_PROPOSALS:-1}
SCHUR_RESIDUAL_PROPOSAL_REBASE_FIRST_ONLY=${SCHUR_RESIDUAL_PROPOSAL_REBASE_FIRST_ONLY:-0}
SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE=${SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE:-0.01}

if [[ -z "$PROPOSAL_ITERATIONS" && "$ANALYZE" == "1" ]]; then
  echo "Oracle-only mode requires ANALYZE=0 and a dedicated analyzer." >&2
  exit 2
fi
...
run_family() {
  ...
  env \
    ...
    SCHUR_ALIGNMENT_DIAGNOSTIC_ITERATIONS="$SCHUR_ALIGNMENT_DIAGNOSTIC_ITERATIONS" \
    SCHUR_KRYLOV_LANDMARK_RESPONSE_ORACLE="$SCHUR_KRYLOV_LANDMARK_RESPONSE_ORACLE" \
    APPLY_SCHUR_KRYLOV_LANDMARK_RESPONSE="$APPLY_SCHUR_KRYLOV_LANDMARK_RESPONSE" \
    ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_ITERATIONS="$PROPOSAL_ITERATIONS" \
    SCHUR_RESIDUAL_PROPOSAL_DIRECTION="$SCHUR_RESIDUAL_PROPOSAL_DIRECTION" \
    ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_REBASE_TRUST_STATE="$ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_REBASE_TRUST_STATE" \
    ...
    SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE="$SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE" \
    SCHUR_PROPOSAL_LANDMARK_RESPONSE_ORACLE="$SCHUR_PROPOSAL_LANDMARK_RESPONSE_ORACLE" \
    APPLY_SCHUR_PROPOSAL_LANDMARK_RESPONSE="$APPLY_SCHUR_PROPOSAL_LANDMARK_RESPONSE" \
    SCHUR_ALIGNMENT_CAMERA_DAMPING="$PROPOSAL_CAMERA_DAMPING" \
    SCHUR_ALIGNMENT_LANDMARK_DAMPING="$PROPOSAL_LANDMARK_DAMPING" \
    SHARED_SCHUR_LANDMARK_REFINEMENT_STEPS="$PROPOSAL_LANDMARK_REFINEMENT_STEPS" \
    ...
    VARIANT_NAME_OVERRIDE="$VARIANT_NAME_OVERRIDE" \
    VARIANT_TAG="landmark_response_i${PROPOSAL_ITERATIONS//,/_i}_${family}" \
    "$RUNNER"
}
...
if [[ "$ANALYZE" == "1" ]]; then
  if [[ "$PROPOSAL_ITERATIONS" == "60,90" \
    && "$SCHUR_RESIDUAL_PROPOSAL_DIRECTION" == "krylov2" \
    && "$ALLOW_TWO_SCHUR_RESIDUAL_PROPOSALS" == "1" \
    && "$SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE" == "0.01" ]]; then
    "$WORKSPACE/serverTest/.venv/bin/python" \
      "$WORKSPACE/serverTest/analyze_k24_krylov_repeat_guard.py" \
      --root "$OUTPUT_ROOT" --i60-root "$I60_REFERENCE_ROOT" \
      --repeated-root "$UNGUARDED_REPEAT_ROOT" \
      --control-root "$CONTROL_ROOT"
    exit 0
  fi
  analysis_args=(
    --proposal-iteration "$PROPOSAL_ITERATIONS"
    --proposal-camera-damping "$PROPOSAL_CAMERA_DAMPING"
    --proposal-landmark-damping "$PROPOSAL_LANDMARK_DAMPING"
    --proposal-landmark-refinement-steps "$PROPOSAL_LANDMARK_REFINEMENT_STEPS"
    --proposal-direction "$SCHUR_RESIDUAL_PROPOSAL_DIRECTION"
  )
  if [[ "$ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_REBASE_TRUST_STATE" == "1" ]]; then
    analysis_args+=(--proposal-trust-rebase)
  fi
  if [[ "$SCHUR_PROPOSAL_LANDMARK_RESPONSE_ORACLE" == "1" && "$APPLY_SCHUR_PROPOSAL_LANDMARK_RESPONSE" == "1" ]]; then
    analysis_args+=(--proposal-landmark-response)
  fi
  "$WORKSPACE/serverTest/.venv/bin/python" \
    "$WORKSPACE/serverTest/analyze_k24_one_step_schur_proposal_breadth.py" \
    --root "$OUTPUT_ROOT" --control-root "$CONTROL_ROOT" \
    --families $FAMILIES "${analysis_args[@]}"
fi
