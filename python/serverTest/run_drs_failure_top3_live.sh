...
if [[ "$CAMERA_UPDATE" != "additive" && "$CAMERA_UPDATE" != "angle_axis_left" && "$CAMERA_UPDATE" != "se3_left" && "$CAMERA_UPDATE" != "se3_right" ]]; then
  echo "CAMERA_UPDATE must be additive, angle_axis_left, se3_left, or se3_right" >&2
  exit 2
fi
...
for problem in "${PROBLEMS[@]}"; do
  ...
  for clusters in $CLUSTERS_LIST; do
    ...
    if [[ -n "$INITIAL_STATE_DIRECTORY" ]]; then
      ...
      initial_state_args+=(--initial-state "$initial_state_path")
    fi
