...
SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE=${SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE:--1}
...
VARIANT_NAME_OVERRIDE=${VARIANT_NAME_OVERRIDE:-}
...
if [[ -n "$ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_ITERATIONS" ]]; then
  ...
  if [[ "$SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE" != "-1" ]]; then
    VARIANT_NAME="${VARIANT_NAME}_repeat_stop${SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE}"
  fi
...
if [[ -n "$VARIANT_NAME_OVERRIDE" ]]; then
  if [[ ! "$VARIANT_NAME_OVERRIDE" =~ ^[a-zA-Z0-9._-]+$ ]]; then
    echo "VARIANT_NAME_OVERRIDE may contain only letters, digits, dots, underscores, and hyphens" >&2
    exit 2
  fi
  VARIANT_NAME="$VARIANT_NAME_OVERRIDE"
fi
...
for problem in "${PROBLEMS[@]}"; do
  ...
  for clusters in $CLUSTERS_LIST; do
    ...
    if [[ "$ONE_STEP_SCHUR_RESIDUAL_PROPOSAL_REBASE_TRUST_STATE" == "1" ]]; then
      ...
      if [[ "$SCHUR_RESIDUAL_PROPOSAL_REBASE_FIRST_ONLY" == "1" ]]; then
        proposal_trust_rebase_args+=(
    ...
    proposal_count_args=()
    if [[ "$ALLOW_TWO_SCHUR_RESIDUAL_PROPOSALS" == "1" ]]; then
      proposal_count_args+=(--allow-two-schur-residual-proposals)
    fi
    two_step_schur_oracle_args=()
    if [[ "$TWO_STEP_SCHUR_RESIDUAL_ORACLE" == "1" ]]; then
    ...
    if [[ "$LIVE_OUTPUT" == "1" ]]; then
      (cd "$SCRIPT_DIR" && /usr/bin/time -v -o "$coordinator_time" \
        ...
        env BUNDLE_PALM_REQUEST_PORT="$REQUEST_PORT" \
            ...
            --schur-repeat-minimum-relative-decrease "$SCHUR_REPEAT_MINIMUM_RELATIVE_DECREASE" \
