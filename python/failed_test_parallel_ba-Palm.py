...
def solveByGDNesterov(Ul, W, Vli, bS, m):
    ...
    for it__ in range(m):
       ...
       if it__ > numMinIterations and stop_criterion(np.linalg.norm(xk, 2), np.linalg.norm(1/Lip * g, 2), it__):
           return xk, it__
    return xk, it__
...
def local_bundle_adjust(
    ...
    while it_ < successfull_its_:
        ...
        if updateJacobian:  # not needed if rejected
            ...
            # todo: verify idea stronger cling on boundary variables to compensate late update.
            # heuristic, must still implement sequential if fails. Can use different acceleration?
            #onlyBorder = vstack((0*J_land_o, J_land_a))
            #onlyBorder = onlyBorder[:,covered_indices_3d]
            # increased to 10/L still huge jump. 
            #penalty_JltJlDiag = JltJlDiag.copy()
            #JltJlDiag += 1 * onlyBorder.transpose() * onlyBorder # better but not solving. Todo check if in general. Also check for acceleration different.
            ####################
            # do not accept update for component? or eval fixed update for 
            # those increasing. or run again non increasing in exra step.
            # 1. identify increasing components in palm solution.
            # 2. set update to 0 there.
            # 3. try acceleration as before.
            # maybe 4. run sequential only on those? keep rest fixed?
            ###################
            # do a special penalty (extra part) ensure it is not affected by L !
            # penalty is not part of JltJlDiag.
            # so L * JltJlDiag + penalty_JltJlDiag
            # extreme?
            # JltJlDiag += 1/L * JltJl.transpose() * JltJl # better but not solving. Todo check if in general. Also check for acceleration different.
        ...
        penaltyL = L * delta_l.dot(JltJlDiag * delta_l) #+ delta_l.dot(penalty_JltJlDiag * delta_l) # TODO another idea. stricter acceptance delta still from original model.
        ...
        print(it_, "it. cost 1     ", round(costEnd), "      + penalty ", round(costEnd + penaltyL + penaltyP), " local cost ", round(np.sum(fx1_o.numpy() ** 2)))
    ...
    return costEnd, x0_p_.numpy(), x0_l_[covered_landmark_indices_,:].numpy(), L_out, L * JltJlDiag + 1e-12 * Vl, powerits_run, delta_old_
...
def updateCluster_palm(
    ...
    cost_, x0_p_c_, x0_l_c_, Lnew_c_, Vl_c_, powerits_run, delta_old_c_ = local_bundle_adjust(
...
primal_cost_vs_old = [0 for elem in range(kClusters)]
...
if basic_version:
    ...
    for it in range(iterations):
        ...
        if run_DRS:
            ...
            primal_cost_v = 0
            for ci in range(kClusters):
                primal_cost_vs_old[ci] = primal_cost_vs[ci]
            ...
            if it > 3:
                # for ci in range(kClusters):
                #     if primal_cost_vs[ci] > primal_cost_vs_old[ci]:
                #         x0_p_new[camera_indices_in_cluster[ci]] = x0_p[camera_indices_in_cluster[ci]].copy()
                #         landmark_v[point_indices_in_cluster[ci]] = landmark_v_old[point_indices_in_cluster[ci]].copy()
                #         primal_cost_vs[ci] = primal_cost_vs_old[ci]

                primal_cost_v = np.sum(primal_cost_vs)

                gains.append(round(old_primal_cost_v - primal_cost_v))
                print("Gains: ", gains)
                print("powerIts: ", powerits_runs)

            print( it, " corrected ====== f(v)= ", round(primal_cost_v), " Gain: ", round(old_primal_cost_v - primal_cost_v), " and ", round(np.sum(cost)), " cost per ci ", primal_cost_vs)
            ...
            if extrapolate_parallel:
                ...
                if base_acceleration:
                    ...
                    #camera_ext_ = x0_p_old + tau * (x0_p_new - x0_p_old) # TODO
                    camera_ext_ = x0_p + tau * (x0_p_new - x0_p)
                    #x0_p_old = x0_p.copy()
                    landmark_v_old = landmark_v.copy()
                else:
                    baseRNA = False
                    if baseRNA:
                        ...
                        #rna_delta  = xk - np.concatenate([x0_p_old.flatten(), landmark_v_old.flatten()]) # TODO
                        rna_delta  = xk - np.concatenate([x0_p_.flatten(), landmark_v_old.flatten()])
                        ...
                        #xk  = np.concatenate([x0_p_old.flatten(), landmark_v_old.flatten()]) # TODO
                        ...
                        if it > 0:
                            ...
                            alpha = 0 #0.1 not sure
                            x_extr = xk1 + wk + alpha * wk1 # wk is old delta. delayed update.
                        ...
                        wk = wk1.copy()
                        xk_old = xk.copy()
                    #x0_p_old = x0_p.copy() # this is provious cam. # TODO
                    landmark_v_old = landmark_v.copy() # that is current landmark.
                ...
                # TODO: local_bundle delivers cost in only relevant residuals
                # to compare with. See where it fails.
