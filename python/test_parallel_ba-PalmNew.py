...
from scipy.sparse import csr_array, csr_matrix, vstack #, issparse
...
def normalize_data(points_3d_, cameras_, desired_scale = 1e8):
...
def read_bal_data(file_name):
    ...
    # skip test examples with/without (<=0) far points!
...
def torchSingleResiduum(camera_params_, point_params_, p2d):
    ...
    points_cam = points_cam + camera_params_[:, 3:6] * c345_mult
...
def torchSingleResiduumX(camera_params, point_params, p2d) :
    ...
    points_cam = points_cam + camera_params[:,3:6] * c345_mult
    points_projX = -points_cam[:, 0] / points_cam[:, 2]
    points_projY = -points_cam[:, 1] / points_cam[:, 2]
...
def torchSingleResiduumScaled(camera_params_, point_params_, p2d, scaling, scalingP):
    ...
    angle_axis = camera_params_[:, :3]
    points_cam = AngleAxisRotatePoint(angle_axis, point_params_)
    points_cam[:,0:2] = points_cam[:,0:2] + camera_params_[:, 3:5]
    points_cam[:,2] = points_cam[:,2] + camera_params_[:, 5]
    points_projX = -points_cam[:, 0] / points_cam[:, 2]
    points_projY = -points_cam[:, 1] / points_cam[:, 2]
    f  = camera_params_[:, 6]
    k1 = camera_params_[:, 7]
...
# unique_poses_in_c_ map from global cam ids to unique local ones such that camera_indices_ act one the correct indices.
def ComputeDerivativeMatricesNewScaled(x0_t_cam, x0_t_land, camera_indices_, point_indices_, torch_points_2d, unique_poses_in_c_, unique_landmarks_in_c_
):
    verbose = False
    if verbose:
        start = time.time() # this is not working at all. Slower then iteratively
...
def blockEigenvalueFull(M, bs, t = 1e-4):
    Ei = M.copy()
    if bs > 1:
        bs2 = bs * bs
...
def copy_selected_blocks(M, block_selection_, bs):
    ...
    if bs > 1:
        bs2 = bs * bs
        for i_ in range(int(M.data.shape[0] / bs2)):
            if block_selection_[i_] == True:
                Mi.data[bs2 * i_ : bs2 * i_ + bs2] = 1e-12
    else:
        Mi = M.copy()
        for i_ in range(int(M.data.shape[0])):
            if block_selection_[i_] == True:
                Mi.data[i_ : i_ + 1] = 1e-12
    return Mi

def stop_criterion(delta, delta_i, it): # maybe lower at later stage?
    eps = 1e-2 #1e-2 used in paper, tune. might allow smaller as faster?
    return (it+1) * delta_i / delta < eps
...
def primal_cost(
    ...
    if not Use_Preconditioning:
        ...
        camScale = 1./Unorm.data.reshape(-1,9)
        camScale = camScale[cameras_indices_in_c_]
        camScale = from_numpy(camScale[camera_indices_[:]])
        camScale.requires_grad_(False)
        ...
        funx0_st1 = lambda X0, X1, X2: torchSingleResiduumScaled(X0.view(-1,9), X1.view(-1,3), X2.view(-1,2), camScale, landScale)
...
def local_bundle_adjust(
    ...
    L = L_in_cluster_
    minimumL = 1e-6
    LipGain = 3 # ? 2 appears bad.
    LfkViolatedOnce = False
    updateJacobian = True
    ...
    if not Use_Preconditioning:
        ...
        # needs for 2 different calls!
        camScale1 = 1./Unorm.data.reshape(-1,9)
        camScale1 = from_numpy(camScale1[unique_cameras_indices_in_c_])
        camScale1.requires_grad_(False)
        # print("camScale ", camScale.shape)
        # print("x0_t_cam ", x0_t_cam.shape)
        # print("cameras ", camScale * x0_t_cam)
        landScale1 = 1./Vnorm.data.reshape(-1,3)
        landScale1 = from_numpy(landScale1[unique_points_in_c_])
        landScale1.requires_grad_(False)
        funx0_st1 = lambda X0, X1, X2: \
            torchSingleResiduumScaled(X0.view(-1, 9), X1.view(-1, 3), X2.view(-1, 2), \
                                    camScale1[local_camera_indices_in_[:]], landScale1[point_indices_in_[:]])
        camScale2 = 1./Unorm.data.reshape(-1,9)
        camScale2 = from_numpy(camScale2[unique_additional_cameras_indices_in_c_])
        camScale2.requires_grad_(False)
        landScale2 = 1./Vnorm.data.reshape(-1,3)
        #landScale2 = from_numpy(landScale2[unique_additional_points_in_c_])
        landScale2 = from_numpy(landScale2[unique_points_in_c_])
        landScale2.requires_grad_(False)
        funx0_st2 = lambda X0, X1, X2: \
            torchSingleResiduumScaled(X0.view(-1, 9), X1.view(-1, 3), X2.view(-1, 2), \
                                    camScale2[additional_local_camera_indices_[:]], landScale2[additional_point_indices_in_[:]])
    ...
    while it_ < successfull_its_:
        ...
        if updateJacobian:  # not needed if rejected
            ...
            if not Use_Preconditioning:
                ...
                J_pose, J_land_o, fx0_o = ComputeDerivativeMatricesNewScaled(
                    x0_t_cam, x0_t_land, local_camera_indices_in_, point_indices_in_, torch_points_2d_in_,
                    unique_cameras_indices_in_c_, unique_points_in_c_,
                )
            ...
            if not Use_Preconditioning:
                ...
                _, J_land_a, fx0_a = ComputeDerivativeMatricesNewScaled(
                    x0_p_a, x0_t_land, additional_local_camera_indices_, additional_point_indices_in_, additional_torch_points_2d_in_,
                        unique_additional_cameras_indices_in_c_, unique_points_in_c_, #unique_additional_points_in_c_,
                )

            if verbose:
                print(J_land_a.data)
                print("min max Ja column indices = landmark index", np.min(J_land_a.indices), " ", np.max(J_land_a.indices))
            ...
            # All steered by 'L', L >= minimumL.
            similar_to_lm_version = True
            if similar_to_lm_version:
                if True:
                    blockEigenvalueJltJl = blockEigenvalue(JltJl, 3) #+ diag_sparse(ones_at_uncovered_indices)
                    #JltJlDiag = JltJl + 1e-3 * blockEigenvalueJltJl
                    blockEigenvalueJtJ = blockEigenvalue(JtJ, 9) # best is 3 replacing 9 -- but BS.
                    #JtJDiag   = JtJ + 1e-4 * blockEigenvalueJtJ

                    # JtJDiag could even be blockEigenvalueJtJ directly?
                    JltJlDiag = 1e-0 * blockEigenvalueJltJl
                    JtJDiag   = 1e-1 * blockEigenvalueJtJ
                else:
                    # much slower.
                    JtJDiag   = blockEigenvalueFull(JtJ, 9, 1e-4)
                    JltJlDiag = blockEigenvalueFull(JltJl, 3, 1e-4)
            else:
        ...
        print(it_, "it. cost 0     ", round(costStart)," cost + penalty ", round(costStart), " === using L = ", L, file=sys.stderr )
        print(it_, "it. cost 0/new ", round(costQuad), " cost + penalty ", round(costQuad + penaltyL + penaltyP), file=sys.stderr)
        ...
        print(it_, "it. cost 1     ", round(costEnd), "      + penalty ", round(costEnd + penaltyL + penaltyP), " local cost ", round(localCost), file=sys.stderr)

        # descent lemma test again. note delta^T nabla = penalty for l,p
        ...
        # LfkDiagonal < -1:
        # 2(const - lin) / quad < -1 ->  (const - lin) < - quad/2  SAFE
        # 2(const - lin) / quad >  2 ->  (const - lin) >   quad    Violation
        # should be ? Haeh ?
        #   (const - lin) <   0        SAFE
        #   (const - lin) >   quad/2   Violation

        else:
            # decent_lemma_divisor = 2
            Lfkconst = costEnd - costStart
            Lfklin = bp.dot(delta_p) + bl.dot(delta_l)
            LfkQuad = (delta_l.dot(nablaXl) + delta_p.dot(nablaXp))
            #LfkDistance = Lfkconst - Lfklin - LfkQuad / 2
            #LfkViolated = LfkDistance > 0 # Lfkconst - Lfklin > LfkQuad/decent_lemma_divisor or (Lfkconst - Lfklin) / LfkQuad > 1/decent_lemma_divisor
            #LfkSafe = Lfklin < 0 # for any phi ok.

            # BASE/Original
            # LfkSafe = Lfkconst - Lfklin < - LfkQuad / 2
            # LfkViolated = Lfkconst - Lfklin > LfkQuad # 476k

            # also ok, maybe correct in theory (performance about same as / 2)
            # LfkViolated = Lfkconst - Lfklin > LfkQuad / 4
            #LfkViolated = Lfkconst - Lfklin > 10 * LfkQuad # does not work so well. So criterion not stupid.
            LfkSafe = Lfkconst - Lfklin < 0 # and above
            # stricter:
            LfkViolated = Lfkconst - Lfklin > LfkQuad / 4
            LfkDiagonal = Lfkconst - Lfklin - LfkQuad / 4

        if costStart < costEnd + penaltyL + penaltyP or LfkViolated:
        #if costStart < costEnd + penaltyL + penaltyP or LfkDiagonal > 2: # TODO maybe also tr-check?
        ...
        print(" ------- Lfk estimate ", LfkDiagonal, " -nabla^Tdelta=" , -bp.dot(delta_p) - bl.dot(delta_l),  " tr_check ", tr_check, " -------- ", file=sys.stderr)
        # update TR -- not now
        if it_ <= successfull_its_:# and L > 1e-6: # lowering leads to, see below averaging affected, can trigger multiple increases
            ...
            #     L = L / 2
            if LfkViolated or costStart < costEnd + penaltyL + penaltyP:
                 L = L * LipGain
            if LfkSafe and LfkViolatedOnce == False: # only decrease if not increased before.
                 L = L / np.sqrt(LipGain)

    # ! growth is slowly propagated to next iteration ! TODO .. really?
    L_out = np.maximum(minimumL, np.minimum(L_in_cluster_ * LipGain, L)) # not clear if generally ok, or 2 or 4 should be used.
    print("LfkDiagonal ", LfkDiagonal, " L ", L, " L_out ", L_out, " L_in_cluster_ ", L_in_cluster_, file=sys.stderr)
...
def GetPcgScalingDiag(JtJ):
    ...
    temp_ = np.fmin(np.fmax(temp_, minTresh), maxTresh)
...
c345_mult = 1; c6_mult = 1; c7_mult = 1; c8_mult = 1
if Use_Preconditioning:
    ...
    fx0 = 1e20
    c345_mult = 20; c6_mult = 3000; c7_mult = 10; c8_mult = 20
    cameras[:,3:6] = cameras[:,3:6] / c345_mult
    cameras[:,6] = cameras[:,6] / c6_mult
    cameras[:,7] = cameras[:,7] / c7_mult
    cameras[:,8] = cameras[:,8] / c8_mult

fx_start = np.sum(fx0**2)
...
lastCost = fx_start
lastCostDRE = fx_start
old_primal_cost_v = fx_start
costs = []
gains = []
powerits_runs = []
acc_gains = []
...
for globalIt in range(iterations):
    if multiCluster:
        ...
        if reCluster and globalIt % 10 == 0: # 3 no gain?! if cost gain stalls?
            (
                camera_indices_in_cluster,
                point_indices_in_cluster,
                points_2d_in_cluster,
    ...
    # " and ", round(np.sum(cost)),
    print( globalIt, "========= f(v)= ", round(primal_cost_v), " Gain: ", round(old_primal_cost_v - primal_cost_v), " cost per ci ", primal_cost_vs)
    if globalIt == iterations-1: #> 3:
    ...
    if extrapolate_parallel:
        ...
        if base_acceleration:
            ...
            if baseRNA:
                ...
                bfgs_ps[globalIt % bfgs_mem] = bfgs_r
                bfgs_qs[globalIt % bfgs_mem] = -(xk1 - xk05 - (xk05 - xk)) # - or + ?
            ...
            landmark_v_old = landmark_v.copy() # TODO: changed
        ...
        if new_line_search: # appears wrong except for the restart idea. But this is to be tackled from new sequential update.
            ...
            line_search_iterations = 3 # 2 is a bit worse, 1 is worse.
            for ls_it in range(line_search_iterations):
                if always_acccept_acceleration:
                    # line_search_iterations=3: 0, 1/3, 2/3 ..
                    # line_search_iterations=2: 0, 1/2
                    tk = ls_it / line_search_iterations # 0, 1/3, 2/3 ..
                else: # regular case
                    tk = ls_it / max(1, (line_search_iterations-1))
                # (1-tk) * (x_new + delta) + tk * x_new = x_new + (1-tk) * delta
                # Goal tk = -1/2, 0. So (tk=1 is just x-new itself)
                #tk = tk - 0.5# extrapolation does not work, eg, 142.
                camera_ext = (1 - tk) * camera_ext_ + tk * x0_p_new
                point_ext = (1 - tk) * point_ext_ + tk * landmark_v
                primal_cost_ext = 0
                primal_costs_ext = [0 for elem in range(kClusters)]
        ...
        if primal_cost_ext < primal_cost_v or (always_acccept_acceleration and primal_cost_ext < 1.1 * np.minimum(primal_cost_v, bestCost) ):
            ...
            # if this happens n consecutive times, reset to avoid bad acceleration.
            if primal_cost_ext > primal_cost_v:
                failedNesterovAcceleration += 1
            else:
                failedNesterovAcceleration = 0

        else: # reset momentum as well?
            for ci in range(kClusters):
                points_3d_in_cluster[ci] = landmark_v.copy()
            x0_p = x0_p_new.copy()

            failedNesterovAcceleration += maxFailedNesterovAcceleration # always reset
        ...
        if use_inertia_in_sequential:
            ...
            if use_inertia_simple: # RNA works, so maybe use here with L*delta, see acceleration
                ...
                if globalIt > 0:
                    beta_nesterov = (globalIt-1) / (globalIt+2)
                    delta_v = xk1 - xk05 + beta_nesterov * delta_v
                    x_extr = xk05 + delta_v
                    camera_ext_ = x_extr[: 9 * n_cameras].reshape(n_cameras, 9)
                    point_ext_ = x_extr[9 * n_cameras :].reshape(n_points, 3)
            ...
            for ls_it in range(line_search_iterations):
                ...
                if primal_cost_ext <= primal_cost_v:
                    primal_cost_vs = [primal_costs_ext[i] for i in range(kClusters)]
                    break
            # TODO maybe need to revert extrapolation if next energy jumped
            # TODO: local_bundle delivers cost in only relevant residuals to compare with. See where it fails.
            print( globalIt, "==== acc. f(v)= ", round(primal_cost_ext), " basic ", round(primal_cost_v), " gain ", round(primal_cost_v-primal_cost_ext), " cost per ci ", primal_costs_ext )
            if globalIt > 1:
                acc_gains.append(round(primal_cost_v-primal_cost_ext))
                if globalIt == iterations - 1:
                    print("acc_gains ", acc_gains)
    ...
    bestCost = np.minimum(np.minimum(primal_cost_ext, primal_cost_v), bestCost)
    bestIt = globalIt
    if globalIt < 60:
        bestCost60 = bestCost
    if globalIt < 30:
