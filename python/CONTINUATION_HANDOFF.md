...
2. Build final tables directly from authoritative JSONL/summary artifacts. Each
  ...
  The final complete-cohort and copied-baseline K1-carryover tables in
  `stage_c_long_horizon_results.tex` are now generated by
  `serverTest/build_stage_c_publication_tables.py` from their authoritative
  summaries. Remaining component tables still require the same conversion.
